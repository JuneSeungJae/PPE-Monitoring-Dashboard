...
이 프로젝트에서는 중앙 **Spring Boot 백엔드 구현**과 **MySQL 기반 DB 설계/매핑**을 담당했습니다. 
- AI 감지 서버와 프론트엔드 대시보드 사이를 연결하는 REST API와 WebSocket 이벤트 흐름을 구현
- 위반 이벤트 저장, 상태 관리, 관리자 인증/권한 처리, 이미지 파일 저장 등을 담당
- 백엔드 데이터 모델과 DB 스키마 설계를 통해 실무형 모니터링 워크플로우를 완성
